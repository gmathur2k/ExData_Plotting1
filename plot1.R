...
## My inputs: data_household_power_consumption.txt, plot1.png, 01/02/2007, 02/02/2007
datfilename=readline(prompt="Enter data file name: ")
plot1filename=readline(prompt="Enter Plot 1 PNG file name: ")
...
## Plot 1: Global Active Power: Histogram of Global Active Power
## pngfilename
png(filename=plot1filename, width=480, height=480, units="px")
par(mfrow=c(1,1), mar=c(5,5,3,0))
hist(hh_power_cons$Global_active_power, col="red", main="Global Active Power", xlab="Global Active Power (kilowatts)")
dev.off()
